...
- [x] Consolidar DTO, caso de uso, query y presenter de opciones asignables en `ROLES`.
  Status: done
...
- [x] Registrar `ROLES/READ_OPTIONS` y derivarla desde `USERS` y `USER_REGISTRATION_INVITATIONS`.
  Status: done
...
- [x] Exponer `GET /v1/roles/options` y delegar rutas legacy en la query de `ROLES`.
  Status: done
...
- [x] Actualizar documentación backend y registrar la transición pendiente de consumidores frontend.
  Status: done
  Cierre: catálogo, reglas de autorización y deuda técnica reflejan el estado implementado.

- [x] Ejecutar verificaciones estáticas y validación manual; ejecutar el seed de roles por la persona usuaria.
  Status: done
