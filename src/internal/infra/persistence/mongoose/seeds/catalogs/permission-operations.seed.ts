import { Model } from 'mongoose';

import { CatalogStatus } from '@domain/entities';
import {
  PermissionOperationDocument,
  PermissionOperationSchema,
} from '@infra/persistence/mongoose/schemas';
import { genId } from '@src/common/utils';

import {
  MongooseSeedContext,
  MongooseSeedDefinition,
  SeedReportItem,
} from '../shared/mongoose-seed.types';

interface PermissionOperationSeedItem {
  code: string;
  name: string;
}

const PERMISSION_OPERATIONS: PermissionOperationSeedItem[] = [
  { code: 'CREATE', name: 'Create' },
  { code: 'READ', name: 'Read' },
  { code: 'UPDATE', name: 'Update' },
  { code: 'DELETE', name: 'Delete' },
];

function getPermissionOperationModel(
  connection: MongooseSeedContext['connection'],
): Model<PermissionOperationDocument> {
  return (
    connection.models[PermissionOperationDocument.name] ??
    connection.model(
      PermissionOperationDocument.name,
      PermissionOperationSchema,
    )
  );
}

export const permissionOperationsSeed: MongooseSeedDefinition = {
  name: 'permission-operations',
  async run(context: MongooseSeedContext): Promise<SeedReportItem> {
    const permissionOperationModel = getPermissionOperationModel(
      context.connection,
    );

    const report: SeedReportItem = {
      name: 'permission-operations',
      created: 0,
      updated: 0,
      unchanged: 0,
    };

    for (const operation of PERMISSION_OPERATIONS) {
      const existing = await permissionOperationModel.findOne({
        code: operation.code,
      });

      if (!existing) {
        await permissionOperationModel.create({
          permission_operation_id: genId(),
          code: operation.code,
          name: operation.name,
          status: CatalogStatus.ACTIVE,
          is_system: true,
        });
        report.created += 1;
        continue;
      }

      let shouldUpdate = false;

      if (existing.name !== operation.name) {
        existing.name = operation.name;
        shouldUpdate = true;
      }

      if (existing.status !== CatalogStatus.ACTIVE) {
        existing.status = CatalogStatus.ACTIVE;
        shouldUpdate = true;
      }

      if (!existing.is_system) {
        existing.is_system = true;
        shouldUpdate = true;
      }

      if (shouldUpdate) {
        await existing.save();
        report.updated += 1;
        continue;
      }

      report.unchanged += 1;
    }

    return report;
  },
};
