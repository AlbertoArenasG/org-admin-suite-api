...
@Injectable()
export class CreateExpirationNotificationPolicyUseCase {
  constructor(
  ...
  async execute(
    input: CreateExpirationNotificationPolicyDto,
    ...
    const policy = new ExpirationNotificationPolicy({
      name: input.name,
      code,
  ...
  private normalizeRules(
    input: CreateExpirationNotificationPolicyDto['rules'],
  ) {
    if (input.length === 0) {
  ...
  private async resolveActiveRecipientGroups(
    rules: ReturnType<
      CreateExpirationNotificationPolicyUseCase['normalizeRules']
    >,
  ): Promise<Map<string, import('@domain/entities').RecipientGroup>> {
